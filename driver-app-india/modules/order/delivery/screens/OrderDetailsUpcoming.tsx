...
import DeliveryAddressUpcoming from '../components/DeliveryAddressUpcoming';
import BillingAddressUpcoming from '../components/BillingAddressUpcoming';
import OrderInstructionUpcoming from '../components/OrderInstructionUpcoming';

export type Props = StackScreenProps<OrderStackParamList, 'order-details'>;

const OrderDetailsUpcoming: React.FC<Props> = () => {
  ...
  // console.log(
  //   '--OrderDetailsUpcoming-singleOrderDetailsId---',
  //   JSON.stringify(singleOrderDetailsId),
  // );
  ...
  useEffect(() => {
    ...
    OrderService.fetchCustomerOrderStatus({
      customer_order_item_id:
        singleOrderDetailsId?.customer_order_items?.[0]?.id,
    });

    OrderService.fetchDeliveryOrderStateFlow({
      customerOrderItemId: singleOrderDetailsId?.customer_order_items?.[0]?.id,
    });
    ...
    <HeaderAvoidingContainer paddingHorizontal={0}>
      <ScrollView
        ...
        <View style={{paddingHorizontal: 16, marginTop: -12}}>
          {orderStateFlow.length ? (
            <DetailsComponent title={''} cardStyle={{minHeight: 200}}>
              <OrderStatusSteps
                ...
                orderDetails={singleOrderDetailsId}
              />
            </DetailsComponent>
          ) : null}
          <Divider height={20} />
          ...
          <ItemsTotal />
          <Divider height={20} />
          <DeliveryAddressUpcoming />
          <Divider height={20} />
          <BillingAddressUpcoming />
          <Divider height={20} />
          <OrderInstructionUpcoming />
          <Divider height={20} />
          <SupportCancelButton
...
export default OrderDetailsUpcoming;
