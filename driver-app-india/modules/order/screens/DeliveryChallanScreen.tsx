...
const DeliveryChallanScreen: React.FC = () => {
  ...
  const createInvoice = async (): Promise<{
    ...
    try {
      ...
      if (invoiceResult.alreadyDelivered) {
        console.log('📋 Invoice handler already marked order as Delivered - will skip markOrderCompleted');
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (invoiceResult.alreadyDelivered) {
        console.log('⏭️ Skipping markOrderCompleted - invoice handler already marked order as Delivered');
      } else {
