...
function AxisPayments(this: any, {route, navigation}) {
  ...
  //   console.log('--axisPayment---request-------', request);

  //   // FAILURE CASE
  //   if (
  //     request?.mainDocumentURL ==
  //       'https://app.fuelbuddy.in/axis-failure?status=FAILED' ||
  //     'https://app.fuelbuddy.in/axis-failure?status=FAILED.verification%20failed'
  //   ) {
  //     navigation.replace('axis-failure');
  ...
  //       }));
  //       navigation.replace('axis-success');
  ...
  const handleShouldStartLoadWithRequest = (request: any) => {
    ...
    if (request?.mainDocumentURL) {
      ...
      if (status && status.startsWith('FAILED')) {
        navigation.replace('axis-failure'); // Navigate to failure screen
        return false; // Prevent further WebView navigation
      }
      ...
      if (status === 'SUCCESS') {
        ...
        navigation.replace('axis-success'); // Navigate to success screen
        return false; // Prevent further WebView navigation
      }
    }
