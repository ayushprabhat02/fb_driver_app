...
const ActiveState: React.FC = () => {
  ...
  useEffect(() => {
    const currentState = statesList.find(
      item => item.name === addressComponents?.state,
    );
    ...
    <View style={{flex: 1}}>
      ...
      <Pressable
        ...
        <Text
          style={{flex: 1}}
          lines={1}
          color={activeState ? 'disabledInputText' : 'neutral'}>
          {selectedState?.name}
        </Text>
